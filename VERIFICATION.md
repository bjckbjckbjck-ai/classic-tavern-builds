...
发布确认：GitHub v0.39.0 已公开并设为最新版；APK/EXE下载地址HEAD返回200，4个附件大小及SHA256与本机完全一致（validation/v0.39.0/github-assets.json）。issue #13 已回复并关闭；本次模拟器和测试主机已停止。源码实现提交76289dc，发布文档与插画提交864e856。

- 12套相关回归，去重210条断言：原11套170条，加reference_v40_test最终40条；其中tomorrow_ui_v37_test在饰品详情改为左图右文后重跑通过，不重复累加。未做平衡模拟。
- 新测试验证15张独立插画、600×800纹理、普通卡回退、完整新图裁切、印刷属性比较仅存在于视图、刷新上移、同UID购买空间轨迹、一次扣款、落回可交互手牌位置、图鉴往返、战斗顶部清除招募控件。
- 原生窗口检查：完整商店/七随从、明日图鉴、饰品详情；生成15张独立插画，经Godot缩放并有损WebP编码，总计2,405,466字节。横向图窗按比例居中裁切，不压扁人物。沿用已有纹理容量限制与重绘缓存，本轮没有新增长局帧率提升结论。
- 最终APK覆盖安装Android API33模拟器，原包名/签名，versionCode41、versionName0.39.0。开启饰品的第8回合，8张手牌/7随从：实际拖到英雄附近和上方间隙成功，金币8→5→2、手牌8→9→10；新图、饰品计数、抬起购买可见。最终修正战斗顶部遮挡后再次安装并复查。
- Android固定结算局面：服务器计算15伤害，4护甲吸收、11生命伤害，对手灰化；英雄冲撞与结算回位录屏保存。此为预置场景、600秒测试时钟，非自然完整对局，录像无音轨。测试结束后关闭本次模拟器和本次测试服务。
- 最终EXE双进程联机、3D视图、法术买入/施放与私有快照通过。APK CRC、专属插画打包、测试素材排除、签名与发布附件哈希检查均保存证据。
- 未验证实体手机长局、热降频、异地网络；模拟器不能代表所有手机帧率。18项差距中，宽屏扇形手牌、窄头像栏、鲍勃壁龛和英雄中心仍明确列为后续。

# v0.38.0 验证记录

发布核验：GitHub latest=v0.38.0，四个附件大小与SHA256匹配本地；APK、EXE下载地址HTTP200。源码功能提交6a90b33，发布内容提交8c4addf。

- 12套相关回归共194条断言通过；最后演出布局调整后重跑experience_v39_test和presentation_3d_test，44条通过；特效预算调整后fusion_3d_test通过。以上为重跑覆盖，不重复累加断言。未做平衡模拟。
- 购买覆盖六个扩展落点、满战场只购买一次、顶部取消、侧栏取消、手牌取消、过期UID、7/8币与满手发现恢复；既有目标施法和3D/2D回退通过。
- 固定24张牌桌面基准：v0.37.1绘制调用444，本版均衡408；平均16.665ms保持60 FPS锁帧，P95由16.752ms到16.729ms。不能据此推断实体手机提升百分比。流畅3D像素数降低43.75%，1倍卡面像素数较旧2倍降低75%；HUD文字不降采样。
- 最终APK安装Android API33模拟器，开启饰品的第8回合，8张手牌/7随从：实际触控拖至英雄附近和手牌上方间隙均成功，金币8→5→2，手牌8→9→10，未误上阵。已检查英雄徽座、大小饰品展示台、饰品详情和画质按钮。
- 最终APK实际录屏检查权威战斗：两张测试随从击败对方后，英雄蓄力冲撞，4护甲被击碎、11生命伤害、对方灰化并淘汰，回位后正常显示战场；伤害由服务器计算15。测试为固定局面/600秒时钟，不是自然完整对局；录像无音轨。
- 最终EXE双进程联机买卖/施法、私有快照通过。APK versionCode40/versionName0.38.0，原包名与原签名一致，覆盖安装成功；CRC正常；验收素材validation/reports未打包。
- Android日志无SCRIPT ERROR或致命异常，首次启动有一次缓存shader重编译警告。未验证实体手机、异地网络、长局热降频；不承诺所有机型已达到60 FPS。

证据：validation/v0.38.0，最终安卓录像另见发布附件。源码与发布文档继续使用3d分支。

# v0.37.1 验证记录

发布核验：GitHub latest=v0.37.1，三个附件大小与SHA256逐项匹配本地；APK与EXE下载地址HTTP200。源码功能提交d1aa600，发布仓库内容提交9b912c3。

- 复现：9张手牌时打开发现，另一个效果补满至10张；旧版本拒绝发现，所有招募操作被发现门禁阻挡，超时自动领取可能无限循环。修复后溢出奖励释放回池，原有手牌保留。
- 8套相关回归、173条断言通过。新规则5条、新UI22条；覆盖2～9回合、7/8币购买、满手与有空位、已准备状态自动结算。没有进行平衡模拟。
- Windows原生UI截图确认提示无重叠；最终EXE双进程网络测试通过。
- 最终APK覆盖安装Android API33模拟器，实际连接测试房间，在第6回合8币满手发现中：点击奖励解除门禁，拖出雄斑虎上场，再拖入青铜守卫，金币8→5，手牌恢复10张。截图与日志见validation/v0.37.1。
- APK版本0.37.1、versionCode39、原包名与签名；CRC与签名验证通过。模拟器验证不代表实体手机或异地网络验证，尚无法确认用户原局唯一触发来源。

# v0.37.0 验证记录

发布核验：GitHub latest=v0.37.0，四附件大小及SHA256匹配本地，APK/EXE HTTP200。#11已回复并关闭（5633155415）。本地源代码提交a3469c0，发布仓库内容提交43f0356。

- 68套功能/UI回归最终逐套日志通过，合计1290条断言；包含44条明日规则检查及5条新UI检查。旧套件按193定义/141在池/24衍生与英雄基础护甲更新夹具，未改低血量AI判定。
- Windows 原生渲染：七奖项转盘、六面骰子、2D回退、卡背交换、明日图鉴和饰品等级详情。Android API33 模拟器实际安装最终APK，连接本机测试房间，显示新随从/手牌、饰品倒计时与奖励等级，播放转盘、骰子、换手牌及结算。日志含 CHANCE 3D VERIFIED wheel/dice Android；没有脚本错误。启动出现一次缓存shader重新编译警告。
- 最终EXE双独立进程联机通过，含私有快照、法术买卖、战斗与断线机器人接管。
- APK versionCode38/versionName0.37.0，最低API24；ZIP1091条目CRC正常，签名沿用 5d15dbad38e4504fa3b1e7ad1aab3c91d0e65bc57f29e8d775866893a659b2cc。
- 验证范围：桌面与安卓模拟器、本机网络链路；未做实体手机、外部异地连接、长时温度/帧率或平衡模拟。录像通过测试房间固定转盘结果，正式版按issue权重随机；无调试强制结果入口。

证据：validation/v0.37.0。旧完整跑中失效夹具的报告由最终逐套重跑覆盖，summary.txt汇总最终日志。

---

# v0.36.0 验证记录

发布核验：GitHub latest=v0.36.0，四附件大小及SHA256匹配本地，APK/EXE/录像HTTP200。#12已回复并关闭（5632579328）；#11已回复计划（5632580017）并保持开放。

- 8套专项189条通过：22位英雄基础护甲/霸主额外10甲、先甲后血；六面朝向、转盘结果、只读、1秒自动返回、切换战斗清理、2D回退和既有暗月/合成规则。
- 原生Windows实际截图检查生成纹理贴模后的转盘、倒角骰子、点数和托盘。修复了网格绕序导致看到相反面的错误，最终六面朝向检查通过。原生专项退出仍有历史SceneTreeTimer存活提示，无SCRIPT ERROR。
- 最终APK在Android13/API33模拟器覆盖安装，点击国王老汉技能掷出4点、获得四星火圈鬼母、消耗5币。基础护甲16可见；随后房主触发夺心护目镜，返回酒馆播放4个法术结果。30币、六本、600秒为测试预置条件。
- 最终Android骰子与转盘均有CHANCE 3D VERIFIED可见像素日志，未见SCRIPT ERROR/FATAL。已检查实际APK截图与20秒可解码录像，不仅检查生成图。
- 最终EXE双进程联机通过。APK1084条目CRC通过、versionCode37、沿用旧签名。未做平衡模拟、实体手机、音效听测或长时温度验证。

证据：validation/v0.36.0。原创图、内置生成方式与完整提示词记录于assets/chance/README.md。

---

# v0.35.0 验证记录

发布核验：GitHub latest=v0.35.0，四附件大小及SHA256匹配本地，APK/EXE/录像HTTP200。已回复#5（issuecomment-5631916134）并确认CLOSED。

- 7套专项回归145条通过：六个骰子正面、六种奖项停靠、只读无二次领奖、自动停留一秒后返回、切换战斗清理、2D兼容、既有暗月规则与国王老汉技能、合成及渲染健康。
- 原生Windows运行专项并检查转盘/骰子截图，独立视口有可见像素确认。专项退出仍有一个SceneTreeTimer存活提示（verbose确认），无脚本错误；不将专项正常退出等同于已证明所有计时器释放。
- 最终APK在Android13/API33模拟器覆盖安装，触控国王老汉技能掷出4点，消耗5币获得四星漂浮观察者；房主随后触发一次转盘，结果神秘魔盒，自动回到酒馆并确认甲板杂兵变金。30币/600秒/六本是预置测试条件。
- Android骰子与转盘均出现CHANCE 3D VERIFIED，日志未见SCRIPT ERROR/FATAL。20秒安装包录像可解码。模拟器验证不等同于实体手机、长时发热或音效听测。
- 最终EXE双进程联机通过，涵盖购买上阵、法术、回放、私有快照与断线BOT接管。不代表双实体设备网络验证。
- APK1080条目CRC通过，versionCode36/versionName0.35.0，沿用旧版证书。没有平衡模拟或全历史组合回归。

证据：validation/v0.35.0。仓库无OPEN issue；#5此前已关闭，本版发布后补充相关验证回复。

---

# v0.34.0 验证记录

发布核验：GitHub latest=v0.34.0、非草稿，四个附件大小与SHA256匹配本地；APK/EXE/录像链接返回HTTP200。
...
证据目录：`validation/v0.34.0`。

---

# v0.33.0 验证记录

发布核验：GitHub latest=v0.33.0、非草稿，四个附件大小与SHA256匹配本地；APK/EXE/录像链接返回HTTP200。
