...
[457项机制/UI、性能样本、最终APK15项触控和EXE同机验证](validation/v0.58.0/README.md)。含模拟器首次启动退出现象和后续3次冷启动结果，不把未复现声称已根治。

# v0.57.0

[413项机制/UI、APK/EXE、帧间隔与边界](validation/v0.57.0/README.md)。最新狼爹单图工作流已接入，#31高级动作与真机验收继续跟踪。

# v0.56.0

[21套899项机制/UI、最终APK与EXE及验证边界](validation/v0.56.0/README.md)。最终EXE房主退出仍有已知资源残留诊断，功能同步通过；没有做平衡模拟。

# v0.55.0

[机制回归、双端UI、最终安装包及验证边界](validation/v0.55.0/README.md)。未做平衡模拟。

# v0.54.0

[16套573项、最终双端包与验证边界](validation/v0.54.0/README.md)。最终APK触控11项通过，最终EXE同机双进程功能通过；房主退出资源诊断仍待修复。未做平衡模拟。
...
见[最终包验证、截图与边界](validation/v0.51.0/README.md)。明日系列暂退，诗心龙与奖励触控修复已进入APK/EXE。模拟器与同机双进程通过，实体设备、音频、外部双机未验证，保留退出资源警告说明。
...
- 此前本版原生卡面回归 271 条及几何检查、23 张截图见 [卡面阶段证据](validation/issue18-card-surfaces/README.md)。本轮完整回归含新卡与新法术；未把同一条重复运行计为新增覆盖。
...
以下为各阶段当时的验证范围；最新发行状态以上文为准。

# v0.50.0 开发阶段：四法术与商店反馈

2026-09-16，尚未发布。[完整日志、截图与边界](validation/issue25-four-spells/README.md)。

- 55个唯一相关功能套件1670断言通过（四法术73）；初次旧界面测试失败的原因和修复复验均保留，不重复累计重跑。
- Windows原生四法术33条、明日之星/转盘5条通过，19张实际截图；手机为Windows布局开关模拟。最后提示位置与倒数顺序另以原生运行确认。
- 同机两进程ENet9条通过，覆盖三张法术跨端状态、刷新、重放拒绝与观战切换；不代表外部两机、Android或完整发行包。
- 规格与质量复查通过；AI换走纳迪娜/低音、交换显示零增益、目标提示叠字、饰品分页进度遗漏已修复。旧测试少量资源释放告警保留。
- 未进行胜率模拟；尚无本版APK/EXE、真机长局性能或音频实听结论。公开包仍0.49，开发协议已0.50。

# v0.50.0 开发阶段：第二组六张与展开血甲

2026-09-16，尚未发布。实现及截图差距见[本阶段证据](validation/issue25-second-six/README.md)。

- 赛博格幼龙、凶饿的邪能幼龙、机械木马、难缠的食尸鱼人、玛诺洛斯、宴会食尸鬼及两级双族衍生物已接入；魔瘾结晶者退役保留定义。207 定义、152 在池、37 法术、22 英雄。
- 42 套相关功能回归 1424 条断言通过，其中第二组专用 85 条。未重跑全部历史套件，未进行胜率平衡模拟。
- Windows OpenGL 原生 UI 32 条通过、18 张截图。动态光环、3D 绘制、实际购买／出牌拖拽、4／10 微扇形、20:9、2D、详情／发现／饰品隐藏及回放血甲时序通过。部分旧短测试退出仍有资源释放告警，原样保存。
- 修复了原始实体详情漏算光环；增加派生显示标记，避免已经绘制的卡面再次叠加。机械木马／小马裁切保留头部；右侧只读血甲牌不改变手牌几何、不截获输入，每帧只读取少量英雄字段。
- 规格、质量独立复查均通过。手机画面仅 Windows 布局开关模拟；本阶段无 APK／EXE、Android、双客户端、长局帧率或音频实听结论。固定 QA 阵容与可见时钟不冒充自然完整对局。
- 四法术、卡框和桌面风格、完整发行仍待完成；#18／#25 保持开放，公开安装包仍 v0.49.0。

# v0.50.0 开发阶段：首组六张与资源反馈

2026-09-16，尚未发布。见[实现、截图对比和完整证据](validation/issue25-first-six/README.md)。

源码实现已本地提交 `8629a2b`，公开文档与证据已推送 `8926d7c`；#18／#25 已回复且保持开放，见 [publication.json](validation/issue25-first-six/publication.json)。本次原生 QA 进程已自行退出，没有启动模拟器或更改电脑网络。

- 正式开发池新增六张历史随从，199 条定义、147 张在池随从、37 张法术；鱼人／亡灵按成员资格各 19 张。
- 42 套相关 headless 功能回归，1331 条断言通过；专用六张套件 94 条。未重跑全部历史套件，也未做平衡模拟。
- 独立规格与质量复查发现并修复：复生回手后售价归属错误、战斗新生成提炼师遗漏售价、宰割预留导致后续铸币回放与结算不符。组合回归覆盖正反结算顺序、去重、真实手牌占位和招募获牌钩子。
- Windows OpenGL 原生 UI 20 条通过、17 张实际截图；拖拽购买一次扣费、生成零费币实际点击使用、3D 纹理与铸币模型、4／10 手牌微扇形和 2D 回退通过。
- 手机仍为 Windows 布局开关模拟；没有本阶段 APK／EXE、Android 或双客户端验收，没有音频实听和性能测量。QA 使用固定阵容、测试驱动停止自动推进与固定可见时钟，不是自然完整对局。
- 商店闲置姓名条与无意义零层数已减少；矩形手牌框、手机展开遮挡英雄和桌面纹理仍需改进。当前 #18／#25 保持开放，第二组六张和四法术仍待实现。
- 原生 UI 日志无脚本错误；部分旧短测试退出仍有资源释放告警，按原样保留日志，不作无告警承诺。

# v0.50.0 开发阶段：种族基础

2026-09-16，尚未发布。见[完整证据与截图差距表](validation/issue25-foundation/README.md)。

- 28 套 headless 功能回归，919 条断言；新增四套112条。128组随机成员图与穷举最优匹配一致；共享池、招募、战斗、AI及既有双端布局功能回归。
- Godot 4.6.1 Windows OpenGL 原生 UI，32 条断言、九张实际截图。磁力拖拽、战吼目标预览及点击提交通过；实际截图发现并修复收藏页上的阶段横幅。
- 对照玩家真实电脑／手机截图，确认星级和种族信息改善，矩形卡框、商店姓名密度、手机遮挡英雄和纹理对比仍未解决。
- 手机仅布局开关模拟；本阶段无新 APK／EXE、双客户端、真机或帧率测量，不进行胜率平衡分析。正式卡数据尚未扩展。

## v0.49.0 验证记录

发布核对：v0.49.0为公开最新版本，APK/EXE直链200、长度和GitHub SHA256匹配；#24完成关闭，#18/#25继续开放。仅停止本轮QA主机与模拟器。
...
发布确认：v0.46.0已公开并设为最新版；APK/EXE完整链接HEAD均返回200，GitHub附件大小与SHA256匹配本地，最终APK与模拟器安装文件一致。issue #19、#20已回复并关闭，#18已回复且保持开放，#17继续保留。本次临时QA主机和模拟器已停止。源码实现提交eafe8be在本机3d，公开文档/证据提交16752fa；源码仓库没有远端，公开仓库提供安装包、文档和证据。校验见github-assets.json、download-check.json及publication.json。
...
发布确认：v0.44.0已公开并设为最新版；APK/EXE完整下载链接HEAD均返回200，附件大小与SHA256匹配本地及模拟器所装APK（见github-assets.json、download-check.json、android-installed.json）。issue #18已回复并保留剩余待办。本次QA主机与模拟器已停止。本地源码实现提交bfd8fcd，公开报告/证据提交0aee3e9；源码仓库未配置远端，公开仓库提供安装包、文档及证据。
...
- 最终APK实际触控：酒馆向最右侧购买第十张牌（8→5币、9→10手牌）；拖给鲍勃出售（5→6币、7→6战场）；拖中间布莱恩上阵（10→9手牌、6→7战场）。服务器快照记录见android-actions.json，保留卡牌ID、阵容顺序与数量变化。
...
发布确认：v0.43.0已公开并设为最新版；APK/EXE完整下载链接HEAD均返回200，两件附件大小与SHA256匹配本地及模拟器所装APK（见github-assets.json、download-check.json、android-installed.json）。issue #18已回复并保留剩余待办。本次QA主机与模拟器已停止。本地源码实现提交1aa2b4d，公开报告/证据提交a9f85e8；源码仓库未配置远端，公开仓库提供安装包、文档及证据。
...
发布确认：v0.42.0已公开并设为最新版，两件附件大小及SHA256与本地一致。公开操作首次遇到网络EOF，确认仍为草稿后重试成功；普通下载URL探测暂为404，追加?download=1的APK/EXE完整下载链接HEAD均返回200。issue #17已回复，音乐资源缺项保持开放。源码1b789d8，公开验证f5a1ef1；本次模拟器和测试主机已停止。

- issue #17部分实现：设置分画面/声音/诊断；帧率30/60/90/120，画质和装饰特效独立；音效/音乐预设音量独立保存。音乐模块此前停用且无启用资源，因此不宣称音乐播放已完成，issue保持开放。
- 设置保持对局运行、显示倒计时；快照到达不重建控件，阶段横幅不覆盖设置。底部中央拱形英雄与技能新布局，结算复用拱形模型。
- 五套54条断言通过：偏好保存、独立设置、对局快照、技能指向、旧购买交互、英雄结算护甲/伤害/灰化、减弱动态等。原生截图验收，修复拱形边框剔除缺失。
- 最终APK覆盖安装API33模拟器，检查帧率选单、音效37%/音乐预设65%、设置倒计时与返回对局，商店/手牌/英雄正常显示。最终EXE双进程联机通过。APK原包名原签名，versionCode45。
- 实体手机长局帧率未验证。鲍勃壁龛、顶部布局、音乐资源继续推进，总体目标未完成。

# v0.41.0 验证记录

发布确认：v0.41.0已公开并设为最新版；APK/EXE下载HEAD均返回200，两件附件大小及SHA256与本地一致。issue #16已回复并关闭；场景整体布局继续按计划推进。本次模拟器和测试主机已停止。源码实现07c09bb，公开文档与验证ccb4095。

- issue #16：新增三处烛火明暗、边缘余烬、按钮高光与点击光环/闪点。环境绘制流畅档15Hz/3余烬，其他档30Hz/7余烬；未增加渲染视口。减弱动态时环境时钟冻结，按钮保留淡出确认。
- 三套33条回归断言通过，原生窗口运行截图检查；装饰不截获输入，不移动按钮点击矩形，不改变规则状态；刷新只扣一次，减弱动态下冻结生效。
- 最终APK覆盖安装API33模拟器，实测刷新8→7并换牌、冻结不扣币。保存三张截图及8秒操作录像。最终EXE双进程联机通过：3D、私有快照、法术购买/施放。APK签名一致，versionCode44。
- 未做实体手机长局性能测试，不宣称帧率提升。鲍勃壁龛与英雄中心尚未实现，总体复刻目标继续。

# v0.40.0 验证记录

发布确认：v0.40.0 已公开并设为最新版，APK/EXE下载HEAD均返回200（APK首次超时、复查成功）；两附件大小及SHA256与本地一致。issue #15 已回复并关闭。13张标记原创的随从资源/格位身份均独立，详见custom-art-audit.json。源码提交e9958c7，公开素材和验证提交60d12b1。本次模拟器和测试主机已停止。

- issue #15 核对有效卡表与呈现覆盖映射：高级灵魂杂耍师/旧杂耍者、复仇下毒鱼人/毒鳍鱼人、小鸽子/高贵鸽子原来共图。本轮四张独立手绘图来自同一2×2图集；明日系列已有独立覆盖，不因source相同误判。深蓝刃鳞的两个ID为同名原版卡，不改原图。
- 修正独立插画误套旧atlas_cell格位，以及椭圆UV拉伸。保留原始图集、格位、提示词与600×600 WebP资源（references/art-v40）。
- 原生窗口截图检查商店/战场/手牌/六星鱼人图鉴/详情。最终APK覆盖安装API33模拟器，查看四张卡的三个区域与长按详情，实测点头像打开对手情报。模拟器不等同实体手机。
- 四套回归79条断言通过；最终Windows EXE双进程联机：私有快照、3D、法术购买施放通过。APK原包名原签名，versionCode43。不进行平衡模拟。
- 后续：鲍勃壁龛、英雄中心、宽屏手牌与实体手机性能；总体复刻目标尚未完成。

# v0.39.1 验证记录

发布确认：GitHub v0.39.1 已公开并设为最新版；APK/EXE 下载地址 HEAD 返回200，两件附件大小与 SHA256 与本机一致（validation/v0.39.1/github-assets.json）。issue #14 已回复并关闭；本次模拟器和测试主机已停止。源码实现提交 aac46a3，公开发布文档提交 d8176d1。issue #15 卡图重复仍待处理。

- issue #14 在修复前通过真实输入事件复现：刷新、冻结点击失败。透明卖牌Panel晚于按钮创建，且区域覆盖顶部按钮；已把卖牌区域起点下移至卡牌行，保留其底部边界。
- 修复后4套相关回归88条断言通过：顶部刷新、冻结、解冻、卖牌，以及原购买、3D表现、图鉴和战斗顶部区域检查。未进行平衡模拟。
- 最终APK覆盖安装Android API33模拟器：第8回合预置局面，点击刷新后金币7→6且商店更换；点击冻结后商店蓝色并显示已冻结，再次点击解冻且金币保持6。保存连续截图。模拟器不等同实体手机验证。
- 最终Windows EXE双进程联机通过：法术购买/施放、私有快照、3D显示。Android原签名、原包名，versionCode42；未改系统电源或网络设置。
- 总体复刻目标仍未完成；继续执行窄头像排名栏、鲍勃壁龛、英雄中心与宽屏交互计划。
