...
def shot(name):Path('screenshots/v44-android-'+name+'.png').write_bytes(run('exec-out','screencap','-p').stdout)
def mode(value):Path('.runtime/v44-device-command.txt').write_text(value,encoding='utf-8');time.sleep(1)
if __name__=='__main__':
    if sys.argv[1]=='shot':shot(sys.argv[2])
    elif sys.argv[1]=='tap':tap(float(sys.argv[2]),float(sys.argv[3]));shot(sys.argv[4])
    elif sys.argv[1]=='swipe':swipe((float(sys.argv[2]),float(sys.argv[3])),(float(sys.argv[4]),float(sys.argv[5])));shot(sys.argv[6])
