...
def shot(name):Path('screenshots/v46-android-'+name+'.png').write_bytes(run('exec-out','screencap','-p').stdout)
def mode(value):Path('.runtime/v46-device-command.txt').write_text(value,encoding='utf-8');time.sleep(1)
