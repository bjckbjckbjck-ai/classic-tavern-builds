"""Local-only ranked spectator check using the preceding live_rooms QA accounts."""
import asyncio,json
...
async def run():
    ...
    async with httpx.AsyncClient(base_url='http://127.0.0.1:18082',timeout=20) as api:
        ...
        async with websockets.connect('ws://127.0.0.1:1500'+str(t['slot']),max_size=16*1024*1024,proxy=None) as ws:
