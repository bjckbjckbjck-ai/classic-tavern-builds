...
状态：服务化预览版0.1.0已实现并部署到 https://bjckwrn.xyz:21111 。游戏规则基于v0.61.0。上线验证范围与限制见[部署报告](docs/DEPLOYED.md)，不把短时测试当作长期稳定性保证。

**当前入口（2026-09-25）：** 按用户要求增加21111 TLS监听，客户端更新为`https://bjckwrn.xyz:21111`，Android版本code=2。该端口已通过外部HTTPS/WSS登录、好友开局及断线托管重连测试。原80/443域名入口曾遭腾讯云webblock拦截，保留历史记录；非80端口不免除ICP备案义务，也不能保证不会被后续拦截，见[腾讯云说明](https://cloud.tencent.com/document/api/243/19630)。
