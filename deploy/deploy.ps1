...
ssh -i $KeyPath ubuntu@bjckwrn.xyz 'sudo python3 /opt/classic-tavern/deploy/enable-port.py'
if ($LASTEXITCODE -ne 0) {throw 'TLS port 21111 setup failed'}
foreach($name in 'ClassicTavern-Service.exe','ClassicTavern-Service.apk') {
...
Invoke-RestMethod https://bjckwrn.xyz:21111/api/health
